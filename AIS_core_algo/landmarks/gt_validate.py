...
def validate(subject: str, landmark: str | None = None) -> None:
    """打印指定 subject 的校验报告。"""
    logger.info("=" * 65)
    logger.info(f"GT Validation — {subject}")
    gt: dict = load_gt(subject)
    features: dict = gt.get("_features", {})
    logger.info(f"  arms={features.get('arms', '?')}  asym={features.get('body_asymmetry', False)}")
    logger.info("=" * 65)
    ...
    for lmk_name, issues in all_results.items():
        ...
        logger.info(f"\n  {lmk_name}: {status}")
        for iss in issues:
            logger.info(f"    [{iss['tag']}] {iss['detail']}")

    logger.info(f"\n  {'─' * 40}")
    logger.info(f"  Total: {total_issues} issue(s)")
