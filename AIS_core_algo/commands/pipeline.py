...
DEFAULT_CONFIG = {
    "steps": [
        ...
        {"name": "feature_eng", "scheme": "v0.1.0"},
...
_DEFAULT_FEATURE_SCHEME = "v0.1.0"  # fallback only when no param given
...
def _load_true_labels() -> tuple[np.ndarray, np.ndarray]:
    df = pd.read_csv("results/extraction/features_extraction/v0.1.0/basic.csv").dropna(subset=["max_cobb"])
...
def _run_train(models: list[str], scheme: str | None, params: dict) -> dict:
    ...
    # ensemble 模式：生成 manuscript 集成预测（复用现有 CompositeV7，不训练）
    if params.get("ensemble"):
        from modeling.ensemble_train import train_ensemble

        feat_scheme = params.get("feature_scheme", _DEFAULT_FEATURE_SCHEME)
        path = train_ensemble(
            scheme_name=feat_scheme,
            model_name=models[0] if models else "HistGBRT",
            hp_n_iter=int(params.get("hp_n_iter", 5)),
        )
        logger.info(f"ensemble 已生成: {path}")
        return {"ensemble": {"path": str(path)}}
