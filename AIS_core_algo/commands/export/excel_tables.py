#!/usr/bin/env python3
"""Table 1-6 的 Excel 表格生成函数，包含所有 make_table 函数及辅助工具。

拆分自 commands.export.excel，本模块仅被 CLI 入口 commands.export.excel 导入。
"""
...
from utils.clinical_stats import load_clinical_stats
from utils.constants import classify_cobb
from utils.logger import logger
from utils.paths import EVAL_TABLES_DIR
...
def make_table1(out_dir: Path | None = None):
    ...
    def ms(gender: str, ais: str, col: str) -> str:
        ...
        return _fmt_median_iqr(vals) if len(vals) else "-"
    ...
    wb.save(target / "table1.xlsx")
    logger.info("Table 1 OK")


def make_table2(out_dir: Path | None = None):
    target = Path(out_dir) if out_dir else EVAL_TABLES_DIR
    df = pd.read_csv(target / "table2_raw.csv")
    ...
    for ri, col in enumerate(cols, 2):
        vals = df[col].dropna().values.astype(float)
        auto = _fmt_median_iqr(vals) if len(vals) else ""
    ...
    for c in range(2, 12):
        ws.column_dimensions[get_column_letter(c)].width = 14
    wb.save(target / "table2.xlsx")
    logger.info("Table 2 OK")


def make_table3(out_dir: Path | None = None):
    target = Path(out_dir) if out_dir else EVAL_TABLES_DIR
    df = pd.read_csv(target / "table3_raw.csv")
    wb = Workbook()
    ws = wb.active
    ws.title = "Table 3"
    ...
    df["severity"] = df["max_cobb"].apply(classify_cobb)
    ...
    for idx_key, idx_label in idx_map.items():
        ...
        for j, sev_name in enumerate(sev_order, 2):
            ...
            val_str = _fmt_median_iqr(vals) if len(vals) else "-"
    ...
    wb.save(target / "table3.xlsx")
    logger.info("Table 3 OK")


def make_table4(out_dir: Path | None = None):
    target = Path(out_dir) if out_dir else EVAL_TABLES_DIR
    df = pd.read_csv(target / "table4_raw.csv")
    wb = Workbook()
    ws = wb.active
    ws.title = "Table 4"
    ...
    wb.save(target / "table4.xlsx")
    logger.info("Table 4 OK")


def make_table5(out_dir: Path | None = None):
    target = Path(out_dir) if out_dir else EVAL_TABLES_DIR
    df = pd.read_csv(target / "table5_raw.csv")
    ...
    wb.save(target / "table5.xlsx")
    logger.info("Table 5 OK")


def make_table6(out_dir: Path | None = None):
    target = Path(out_dir) if out_dir else EVAL_TABLES_DIR
    df = pd.read_csv(target / "table6_raw.csv")
    y_true = df["max_cobb"].values.astype(float)
    y_pred = df["pred_max_cobb"].values.astype(float)
