...
因 paper_figures_panels 存在 render_scatter，扁平导出会歧义）。

按用途分组：

网格域（mesh / ROI 展示）
    cut_panels                render_mesh_panel / render_roi_extract_panel / render_single_panel ...
    landmarks_panels          render_curvature_landmarks_panel
    parameterization_panels   draw_cut
    moire_panels              render_moire
