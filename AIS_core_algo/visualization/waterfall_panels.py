...
def render_waterfall(
    ...
    *,
    ...
    ``true_cobb`` 为 None 时不画 Ground truth 虚线（预测/推理场景无 GT）。
    """
    ...
    # 8) Limits
    y_max = max(expected_val, pred_val) + 3
    if true_cobb is not None:
        y_max = max(y_max, true_cobb + 3)
