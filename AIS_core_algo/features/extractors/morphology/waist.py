...
def extract_waist(
    ...
    # ── waist ──
    left: np.ndarray = np.asarray(landmarks["waist_L"])
    right: np.ndarray = np.asarray(landmarks["waist_R"])
    ...
    wa_y: float = float(np.mean([left[1], right[1]]))
    if profiles is not None:
        ...
        left_lower = np.array([float(left[0]), wa_y - 80.0, float(left[2])])
        right_lower = np.array([float(right[0]), wa_y - 80.0, float(right[2])])
