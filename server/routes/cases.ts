...
async function createCase(user: any, body: any) {
  ...
  if (!body?.name || !body?.gender || !body?.height || !body?.weight) throw new Error("Name, gender, height and weight are required.");
  const count = await db.case.count();
  return db.case.create({ data: { caseNumber: `CASE${String(count + 1).padStart(6, "0")}`, name: body.name, gender: body.gender, birthDate: body.birthDate ? new Date(body.birthDate) : null, heightCm: Number(body.height), weightKg: Number(body.weight), medicalHistory: body.medicalHistory, department: body.department, doctor: body.doctor, ownerId: user.id, institutionId } });
}
