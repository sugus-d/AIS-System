...
const router = Router();
const publicUser = (user: any) => ({ id: user.id, username: user.username, name: user.displayName, role: user.role, department: user.department, email: user.email, phone: user.phone });
router.post("/login", async (req: Request, res: Response) => {
  const { username, password } = req.body || {}; const user = await db.user.findUnique({ where: { username } });
...
router.get("/me", async (req: Request, res: Response) => { const userId = parseToken(req.headers.authorization?.replace("Bearer ", "")); const user = userId ? await db.user.findUnique({ where: { id: userId } }) : null; if (!user || !user.active) return res.status(401).json({ success: false, message: "登录已失效" }); return res.json({ success: true, data: publicUser(user) }); });
