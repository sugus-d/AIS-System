...
router.post("/login", async (req: Request, res: Response) => {
  const { username, password } = req.body || {}; if (typeof username !== "string" || !username || typeof password !== "string" || !password) return res.status(400).json({ success: false, message: "请输入用户名和密码" }); const user = await db.user.findUnique({ where: { username }, include: { institution: true } });
