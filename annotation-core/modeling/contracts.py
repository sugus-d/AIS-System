...
# 训练结果根目录（契约层常量，避免 contracts → modeling.training 反向依赖）
# 注意：此为训练结果子目录（非 results 根；commands/export/config.py 的 RESULTS_DIR 是根目录，两者勿混）
RESULTS_DIR = Path("results/modeling/prediction")
