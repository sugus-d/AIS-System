...
from utils.geometry import to_2d
from utils.signal_ops import smooth_contour

from ..constants import ShoulderTransition
from .debug import _empty_debug
...
def detect_shoulder_transition(
    ...
    """按 spec 搜索左右肩臂转点，返回 (2,2) 的 2D 点与 debug dict。
    ...
    Returns:
        Tuple[np.ndarray, dict]: shoulder_transition (2,2), debug dict
    """
    ...
    for side_name, contour2, ax_y, has_arm, inner_x, inner_y, side_idx in [
        ...
        if arm_boundary_x is None or len(contour2) < ShoulderTransition.MIN_SEGMENT_LEN:
            ...
            debug[side_name] = _empty_debug()
            debug[side_name]["fallback"] = True
            debug[side_name]["outer_x"] = None
            debug[side_name]["inner_x"] = float(inner_x)
            continue
        ...
        peak_point = smoothed[peak_index].copy()
        peak_angle = float(long_axis_angles[peak_index])

        results.append(peak_point)
        debug[side_name] = {
            "contour": smoothed,
            "long_axis_angles": long_axis_angles,
