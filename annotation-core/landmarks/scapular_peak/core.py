...
from .debug import build_scapular_peak_debug
...
def detect_scapular_peak(
    ...
    left_peak, left_debug = _detect_one_side(
        band,
        band_indices,
        True,
        mid_x,
    ...
    right_peak, right_debug = _detect_one_side(
        band,
        band_indices,
        False,
        mid_x,
    ...
    debug = build_scapular_peak_debug(
        y_lo,
        y_hi,
        mid_x,
        band_size,
        ...
        if high_z_size < ScapularPeak.MIN_HIGH_Z_SIZE:
            ...
            if target_y is not None:
                ...
                if (
                    ...
                    candidate_indices = np.array([cand_indices_all[best_local_idx]])
                else:
                    peak = candidates[np.argmax(candidates[:, 1])]

    curvature_used = curvature is not None and not fallback and target_y is None
    debug = {
        "side_size": side_size,
        ...
        "fallback": fallback,
        "curvature_used": curvature_used,
        "curvature_at_peak": float(curvature[candidate_indices[0]]) if curvature_used else 0.0,
    }
    return peak, debug
