...
from .debug import build_spine_debug
...
def fit_spine_midline(
    ...
    Returns:
        (midline, debug): midline 为 (200, 3) 平滑中线，debug 包含候选点及过滤过程。
    """
    ...
    raw_pts: np.ndarray = np.array(mid_points) if mid_points else np.zeros((0, 3))

    if len(mid_points) < poly_deg + 2:
        return np.zeros((0, 3)), build_spine_debug(raw_pts, mid_x, n_bins)
    ...
    midline: np.ndarray = np.column_stack([spl_x2(y_smooth), y_smooth, spl_z2(y_smooth)])
    return midline, build_spine_debug(pts, mid_x, n_bins, rejected_mad, rejected_residual, pts_clean)
