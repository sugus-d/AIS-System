...
def detect_waist(
    ...
    Returns:
        Tuple of (waist_points (2, 2), debug_dict).
    """
    ...
    li, l_angle = _pick_most_vertical(left_c, target_y)
    ri, r_angle = _pick_most_vertical(right_c, target_y)

    debug = {
        "y_centers": y_cen,
        "widths_raw": widths,
        "widths_smooth": w_full_smooth,
        "y_lo": y_lo,
        "y_hi": y_hi,
        "target_y": target_y,
        "left_angle_deg": l_angle,
        "right_angle_deg": r_angle,
        "left_idx": li,
        "right_idx": ri,
    }
    return np.stack([left_c[li], right_c[ri]]), debug
...
def _pick_most_vertical(
    ...
    """在 target_y ± _WINDOW_HALF_WIDTH 窗口内选长轴角最小（最垂直）的轮廓点。

    WHY：腰线最窄处的垂直度是定位金标准。
    在 target_y 附近用 ±30mm 窗口搜索，既包容左右不对称又不过度偏离最窄 Y。

    Returns:
        (best_index, best_angle_deg)。无候选时退回到最近点，angle=-1。
    """
    y_vals = contour[:, 1]
    ...
    if len(candidates) == 0:
        ...
        best = int(np.argmin(np.abs(y_vals - target_y)))
        return best, -1.0
    angles = [_vertical_angle_at(contour, i) for i in candidates]
    best_local = int(np.argmin(angles))
    return int(candidates[best_local]), angles[best_local]
